...
function send-message {
    ...
    if($typeMessage -eq 1) {
        $MessageBody = "<h3><b><font color=green>Установка доступа к почтовому ящику!</b></font></h3> <br>"
        $MessageBody += "<p><b>Инициатор заявки:</b> $script:nameInitiator<br>"
        $MessageBody += "<p><b>По вашему запросу установлен доступ к ящику</b> $script:nameSender <b> сотрудником </b> $script:nameRecipient <br>"
        $MessageBody += "<br>"
        $MessageBody += "<br>"
    } else {
        $MessageBody = "<h3><b><font color=green>Установка доступа к почтовому ящику!</b></font></h3> <br>"
        $MessageBody += "<p><b>По вашему запросу установленый доступ к ящику снят:</b> $script:nameSender<br>"
        $MessageBody += "<br>"
        $MessageBody += "<br>"
    }
...
function Set-Forwarding {
    ...
    if ($null -ne ($txtFiles=Get-ChildItem $pathFiles\*.* -Include *.txt | Sort-Object LastWriteTime)) { 
        ...
        $txtFiles | ForEach-Object {
            ...
            if($type -eq 'set') {
                Add-MailboxPermission -Identity $recipientMail -User $senderMail -AccessRights FullAccess -AutoMapping:$true -InheritanceType All
                send-message -typeMessage 1
                $_ | Move-Item -Destination $ProcessedRequests  
            }
            if(($type -eq 'clear')) {
                Remove-MailboxPermission -Identity $recipientMail -accessrights:fullaccess -User $senderMail
